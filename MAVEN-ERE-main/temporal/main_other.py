...
from data_other import REL2ID_DICT, EVAL_EXCLUDE, NONE_REL_DICT
import os
import warnings
import sys
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataname", default="MATRES", type=str)
    ...
    assert args.ignore_nonetype, print("please set --ignore_nonetype")
    ...
    EVAL_EXCLUDE_ID = [REL2ID[r] for r in EVAL_EXCLUDE[args.dataname.lower()] if r in REL2ID]
    ...
    if not args.eval_only:
        train_dataloader = get_dataloader(tokenizer, "train", args.dataname, data_dir=data_dir, max_length=256, shuffle=True, batch_size=args.batch_size, ignore_nonetype=args.ignore_nonetype, istrain=True)
        dev_dataloader = get_dataloader(tokenizer, "dev", args.dataname, data_dir=data_dir, max_length=256, shuffle=False, batch_size=args.batch_size, ignore_nonetype=args.ignore_nonetype, istrain=False)
    test_dataloader = get_dataloader(tokenizer, "test", args.dataname, data_dir=data_dir, max_length=256, shuffle=False, batch_size=args.batch_size, ignore_nonetype=args.ignore_nonetype, istrain=False)
